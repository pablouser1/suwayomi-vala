public class Api {
    ...
    public Api (string base_url, string? username, string? password) {
    ...
    public async Gee.List<MangaTypeBasic> mangas_from_category (int64 category_id) throws Error {
        ...
        for (int i = 0; i < nodes_array.get_length (); i++) {
            ...
            var thumbnail_url = category_obj.get_string_member ("thumbnailUrl");

            var manga = new MangaTypeBasic (id, title, thumbnail_url);
    ...
    public async MangaType manga (int64 manga_id) throws Error {
        ...
        var thumbnail_url = manga.get_string_member ("thumbnailUrl");

        return new MangaType (manga_id, title, description, thumbnail_url);
    }
    ...
    public async Gee.List<ChapterType> chapters (int64 manga_id) throws Error {
        ...
        // Order
        var order_obj = new Json.Object ();
        order_obj.set_string_member ("by", "SOURCE_ORDER");
        order_obj.set_string_member ("byType", "DESC");
        var order = new Json.Array ();
        order.add_object_element (order_obj);

        // Variables
        var variables = new Json.Object ();
        variables.set_object_member ("condition", condition);
        variables.set_object_member ("order", order_obj);
        ...
        for (int i = 0; i < nodes_array.get_length (); i++) {
            ...
            var is_read = chapter_obj.get_boolean_member ("isRead");
            var last_page_read = chapter_obj.get_int_member ("lastPageRead");
            var scanlator = chapter_obj.get_string_member_with_default ("scanlator", "??");
            var upload_date = chapter_obj.get_string_member ("uploadDate");

            var chapter = new ChapterType (id, name, is_read, last_page_read, scanlator, upload_date);
    ...
    private async Json.Object graphql (string query, Json.Object? variables = null) throws Error {
        ...
        // Send request
        var message = new Soup.Message ("POST", this.base_url + "/api/graphql");
        ...
        try {
            ...
            throw new Error (
                Quark.from_string ("JSON parse error"),
                0,
                "Failed to parse GraphQL response: %s", e.message
            );
        }
